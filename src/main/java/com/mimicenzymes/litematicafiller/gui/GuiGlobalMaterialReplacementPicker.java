...
public class GuiGlobalMaterialReplacementPicker extends GuiBase {
    ...
    protected void drawContents(GuiContext drawContext, int mouseX, int mouseY, float partialTicks) {
        ...
        if (hoveredIndex >= 0) {
            ...
            lines.add(hoveredStack.getHoverName().getString());
            ...
            target.ifPresent(stack -> lines.add(StringUtils.translate(
                    ...
                    stack.isEmpty() ? StringUtils.translate("litematica_container_filler.gui.label.material_replace_ignored") : stack.getHoverName().getString()
            )));
    ...
    private void drawParent(GuiContext drawContext, int mouseX, int mouseY, float partialTicks) {
        if (this.parent != null) {
            this.parent.extractRenderState(drawContext, mouseX, mouseY, partialTicks);
        }
    }
