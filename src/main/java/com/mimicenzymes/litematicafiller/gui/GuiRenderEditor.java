...
public class GuiRenderEditor extends GuiBase {
    ...
    private int addColorControl(int x, int y, int width, ConfigColor config) {
        ...
        GuiTextFieldGeneric field = new GuiTextFieldGeneric(layout.valueX + 3, y + 2, Math.max(24, layout.valueW - 6), 16, this.font);
    ...
    protected void drawScreenBackground(GuiContext GuiContext, int mouseX, int mouseY) {
        drawGradient(GuiContext, 0, 0, this.getScreenWidth(), this.getScreenHeight(), BACKGROUND_TOP, BACKGROUND_BOTTOM);
        GuiContext.fill(0, 0, this.getScreenWidth(), 68, APP_BAR);
        GuiContext.fill(0, 67, this.getScreenWidth(), 68, OUTLINE);
        GuiContext.fill(0, 68, this.getScreenWidth(), 100, 0x26000000);
    }
    ...
    public boolean onMouseClicked(MouseButtonEvent MouseButtonEvent, boolean doubleClick) {
        ...
        if (handleColorInputClick(MouseButtonEvent, doubleClick)) {
            return true;
        }
        if (MouseButtonEvent.button() == 0 && handleChromeClick(mouseX, mouseY)) {
            return true;
        }

        Rect preview = getPreviewBounds();
        if (MouseButtonEvent.button() == 0 && isPointInside(mouseX, mouseY, preview.x, preview.y, preview.width, preview.height)) {
    ...
    public boolean onKeyTyped(KeyEvent KeyEvent) {
        ...
        if (focused != null) {
            ...
            if (focused.field.keyPressedWrapper(KeyEvent)) {
    ...
    @Override
    public boolean onCharTyped(CharacterEvent CharacterEvent) {
        ColorInputBinding focused = getFocusedColorInput();
        if (focused != null && focused.field.charTypedWrapper(CharacterEvent)) {
            applyColorInput(focused);
            return true;
        }
    ...
    protected void drawContents(GuiContext GuiContext, int mouseX, int mouseY, float partialTicks) {
        ...
        drawMaterialTitle(GuiContext);

        EditorLayout layout = getEditorLayout();

        drawCard(GuiContext, layout.previewPanel.x, layout.previewPanel.y, layout.previewPanel.width, layout.previewPanel.height);
        drawCard(GuiContext, layout.controlsPanel.x, layout.controlsPanel.y, layout.controlsPanel.width, layout.controlsPanel.height);
        drawSectionHeader(GuiContext, tr("litematica_container_filler.gui.label.render_editor_preview"), layout.previewPanel.x + 16, layout.previewPanel.y + 12);
        drawSectionHeader(GuiContext, tr("litematica_container_filler.gui.label.render_editor_settings"), layout.controlsPanel.x + 16, layout.controlsPanel.y + 12);
        drawTabDescription(GuiContext, layout.controlsPanel.x + 16, layout.controlsPanel.y + 31, layout.controlsPanel.width - 32);
        drawPreview(GuiContext, layout.preview.x, layout.preview.y, layout.preview.width, layout.preview.height, partialTicks);
        drawRenderWarning(GuiContext, layout.previewPanel.x + 14, layout.previewPanel.y + layout.previewPanel.height - 50, layout.previewPanel.width - 28);
        super.drawContents(GuiContext, mouseX, mouseY, partialTicks);
        drawTabButtonOverlay(GuiContext, mouseX, mouseY);
        drawBackButtonOverlay(GuiContext, mouseX, mouseY);
        drawTabControlsOverlay(GuiContext, mouseX, mouseY);
    }

    private void drawTabDescription(GuiContext context, int x, int y, int width) {
    ...
    private void drawRenderWarning(GuiContext context, int x, int y, int width) {
    ...
    private void drawMaterialTitle(GuiContext context) {
    ...
    private void drawSectionHeader(GuiContext context, String label, int x, int y) {
        drawString(context, label, x, y, TEXT);
        context.fill(x, y + 13, x + 28, y + 15, PRIMARY);
        context.fill(x + 29, y + 13, x + 44, y + 15, PRIMARY_CONTAINER);
    }

    private void drawTabButtonOverlay(GuiContext context, int mouseX, int mouseY) {
        int x = 18;
        int y = 46;
        for (EditorTab value : EditorTab.values()) {
            ...
            drawString(context, tr("litematica_container_filler.gui.button.render_editor." + value.key), x + 10, y + 6, textColor);
            if (selected) context.fill(x + 10, y + 18, x + 76, y + 20, PRIMARY);
            x += 90;
        }
    }

    private void drawBackButtonOverlay(GuiContext context, int mouseX, int mouseY) {
    ...
    private boolean handleColorInputClick(MouseButtonEvent MouseButtonEvent, boolean doubleClick) {
        boolean clickedInput = false;
        for (ColorInputBinding input : this.colorInputs) {
            boolean handled = input.field.mouseClickedWrapper(MouseButtonEvent, doubleClick);
    ...
    private void drawTabControlsOverlay(GuiContext context, int mouseX, int mouseY) {
        List<ControlSpec> controls = getControlsForTab();
        ControlLayout layout = getControlLayout(controls.size());
        for (int i = 0; i < controls.size(); i++) {
            ControlRect rect = layout.rectFor(i);
    ...
    private int drawBooleanOverlay(GuiContext context, int mouseX, int mouseY, int x, int y, int width, ConfigBoolean config) {
        return drawButtonOverlay(context, mouseX, mouseY, x, y, width, booleanLabel(config));
    }

    private int drawButtonOverlay(GuiContext context, int mouseX, int mouseY, int x, int y, int width, String label) {
    ...
    private int drawStepperOverlay(GuiContext context, int mouseX, int mouseY, int x, int y, int width, String label) {
        int resetW = Math.min(54, Math.max(44, width / 4));
        int valueW = Math.max(70, width - resetW - 86);
    ...
    private int drawColorOverlay(GuiContext context, int mouseX, int mouseY, int x, int y, int width, String label, ConfigColor config) {
        ColorControlLayout layout = getColorControlLayout(x, y, width);
    ...
    private void drawPreview(GuiContext context, int x, int y, int width, int height, float partialTicks) {
    ...
    private void drawHighlightPreview(GuiContext context, int x, int y, int width, int height) {
    ...
    private void drawMarkerPreview(GuiContext context, int x, int y, int width, int height) {
    ...
    private void drawHudPreview(GuiContext context, int x, int y, int width, int height) {
    ...
    private void drawHudCard(GuiContext context, int panelX, int panelY, int panelW, int panelH, int alpha) {
        ...
        int textHeight = Math.max(1, (int)Math.ceil(this.fontHeight * scale));
    ...
    private void drawRoundedPreviewCard(GuiContext context, int x, int y, int width, int height, int color) {
    ...
    private void drawRoundedPreviewCardSection(GuiContext context, int x, int y, int width, int height, int sectionTop, int sectionBottom, int color) {
    ...
    private void drawRoundedPreviewCardOutline(GuiContext context, int x, int y, int width, int height, int color) {
        if (width <= 2 || height <= 2) {
            return;
        }
    ...
    private void drawRoundedPreviewRows(GuiContext context, int x, int y, int width, int height, int radius, int fromRow, int toRow, int color) {
    ...
    private void drawHighlightModel(GuiContext context, int cx, int cy, float size, Color4f color, boolean glass, boolean topPlate) {
        if (glass) {
    ...
    private void drawMarkerModel(GuiContext context, int cx, int cy, float size, double time, MarkerKind kind) {
        switch (kind) {
    ...
    private void drawPreviewDownArrow(GuiContext context, int cx, int cy, float halfWidth, float height, float depth, Color4f color) {
    ...
    private void drawPreviewSpinner(GuiContext context, int cx, int cy, float size, double time, Color4f color) {
        for (int i = 0; i < 8; i++) {
    ...
    private void drawPreviewExclamation(GuiContext context, int cx, int cy, float size, Color4f color) {
    ...
    private void drawCrosshairReference(GuiContext context, int cx, int cy) {
    ...
    private void fillRotatedRect(GuiContext context, int cx, int cy, int relX1, int relY1, int relX2, int relY2, int color, ArrowDirection direction) {
    ...
    private void fillRotatedPolygon(GuiContext context, int cx, int cy, ArrowDirection direction, int color, int[] relXs, int[] relYs) {
    ...
    private void fillPolygon(GuiContext context, int[] xs, int[] ys, int color) {
    ...
    private void drawPreviewCuboid(GuiContext context, int cx, int cy, float size, Color4f color) {
        drawPreviewBox(context, cx, cy, size, size, size, color);
    }

    private void drawPreviewPrism(GuiContext context, float[] xs, float[] ys, float halfZ, Color4f color) {
    ...
    private void drawPreviewBox(GuiContext context, float cx, float cy, float halfX, float halfY, float halfZ, Color4f color) {
    ...
    private void drawPreviewWorldBox(GuiContext context, float cx, float cy, float scale, float minX, float minY, float minZ, float maxX, float maxY, float maxZ, Color4f color) {
    ...
    private void drawPreviewWorldBox(GuiContext context, List<PreviewDrawCommand> commands, float cx, float cy, float scale, float minX, float minY, float minZ, float maxX, float maxY, float maxZ, Color4f color) {
    ...
    private void drawPreviewBox(GuiContext context, float cx, float cy, float centerX, float centerY, float centerZ, float halfX, float halfY, float halfZ, Color4f color, List<PreviewDrawCommand> commands) {
        double yaw = Math.toRadians(this.previewYaw);
        double pitch = Math.toRadians(this.previewPitch);
        double cosY = Math.cos(yaw);
        double sinY = Math.sin(yaw);
    ...
    private void fillQuad(GuiContext context, int[] xs, int[] ys, int[] face, int color) {
        fillQuad(context, xs, ys, face, color, null);
    }

    private void fillQuad(GuiContext context, int[] xs, int[] ys, int[] face, int color, List<PreviewDrawCommand> commands) {
        int minY = Integer.MAX_VALUE;
        int maxY = Integer.MIN_VALUE;
        for (int index : face) {
    ...
    private void drawCard(GuiContext context, int x, int y, int width, int height) {
    ...
    private void drawGrid(GuiContext context, int x, int y, int width, int height) {
    ...
    private void drawGradient(GuiContext context, int x1, int y1, int x2, int y2, int top, int bottom) {
        context.fillGradient(x1, y1, x2, y2, top, bottom);
    }

    private void drawSoftRect(GuiContext context, int x, int y, int width, int height, int color) {
        if (width <= 0 || height <= 0) return;
        context.fill(x, y, x + width, y + height, color);
    }

    private void drawRoundedRect(GuiContext context, int x, int y, int width, int height, int radius, int color) {
        if (width <= 0 || height <= 0) return;
        int r = clamp(radius, 0, Math.min(width, height) / 2);
        if (r <= 0) {
    ...
    private void drawRoundedOutline(GuiContext context, int x, int y, int width, int height, int radius, int border, int fill) {
    ...
    private void drawMaterialButton(GuiContext context, int x, int y, int width, int height, String label, boolean hovered, boolean primaryText) {
    ...
    private void drawPreviewLabel(GuiContext context, String label, int cx, int y, int cellWidth) {
    ...
    private void drawLine(GuiContext context, int x1, int y1, int x2, int y2, int color) {
        drawLine(context, x1, y1, x2, y2, color, null);
    }

    private void drawLine(GuiContext context, int x1, int y1, int x2, int y2, int color, List<PreviewDrawCommand> commands) {
    ...
    private void drawScaledString(GuiContext context, String Component, int x, int y, float scale, int color) {
        if (Component == null || Component.isEmpty()) {
            return;
        }
        if (Math.abs(scale - 1.0f) < 0.01f) {
            drawString(context, Component, x, y, color);
            return;
        }
        ...
        drawString(context, Component, 0, 0, color);
        matrices.popMatrix();
    }
    ...
    private String fit(String Component, int maxChars) {
        if (Component == null || Component.length() <= maxChars) return Component;
        if (maxChars <= 1) return Component.substring(0, 1);
        return Component.substring(0, maxChars - 1) + "...";
    }
    ...
    private String fitToWidth(String Component, int maxWidth) {
        if (Component == null || Component.isEmpty() || maxWidth <= 0) {
            return "";
        }
        ...
        while (end > 0 && this.font.width(Component.substring(0, end)) + suffixWidth > maxWidth) {
            end--;
        }
        return end <= 0 ? suffix : Component.substring(0, end) + suffix;
    }
    ...
    private List<String> wrap(String Component, int maxChars) {
        if (Component == null || Component.isEmpty()) return List.of();
        java.util.ArrayList<String> lines = new java.util.ArrayList<>();
        for (String paragraph : Component.split("\\n")) {
